...
This page validates `scDiagnostics` core functionality using controlled simulations. Synthetic single-cell data with known ground truth allows us to:

- Verify anomaly detection works correctly on well-behaved data (baseline)
- Test diagnostic sensitivity when reference is incomplete (missing cell type)
- Establish baseline metrics before applying to real biological data
- Understand how diagnostic functions respond to controlled perturbations

## Simulation Data Generation

We use [**splatter**](https://bioconductor.org/packages/splatter) to generate realistic synthetic scRNA-seq data~\cite{zappia17}. Splatter is a statistical simulation framework that generates single-cell RNA-seq count matrices with user-specified parameters including cell type structure, batch effects, differential expression, and gene-gene correlations. This allows for ground-truth validation where the true cell type identity of every cell is known—impossible with real biological data.

**Why synthetic data for validation?**
...
- **Baseline establishment** — Determines what "normal" anomaly detection looks like before analyzing complex real data
...
Query cells are annotated using [**SingleR**](https://bioconductor.org/packages/SingleR)~\cite{aran19}, a reference-based annotation tool that correlates query cell expression against reference pseudobulk profiles. SingleR provides a practical test case for evaluating annotation quality diagnostics.
