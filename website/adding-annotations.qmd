...
This page describes how to add cell type annotations from four popular tools (Azimuth, SingleR, CellTypist, and scArches) to your processed data. The workflow applies these tools to assign cell types to cells in your query dataset using reference data as a guide.
...
For each annotation method, the workflow follows these steps:
...
**Script:** `R/covid/Create Azimuth Reference.R`

**Key steps:**

- Loads healthy reference data
- Runs SCTransform (standardized normalization)
...
- 3,000 variable features (standard for reproducibility)
- 50 PCA dimensions
...
**Note:** This step only needs to run once per reference dataset.
...
- `R/covid/Add R (Azimuth and SingleR) Annotations.R` — Main execution script (also runs SingleR)
...
- 4 annotation systems (Azimuth L1, Azimuth L2, SingleR, CellTypist, scArches)
