...
This page explores annotation quality in COVID-19 PBMC data by comparing healthy reference cells to severe COVID-19 patient cells. We use a combination of dimensionality reduction, gene expression analysis, and anomaly detection to understand how transcriptomic changes affect cell type annotation.

## Dataset Context

- **Healthy Reference:** 23,201 PBMC cells from 23 patients
...
- **Note:** Results below use `Azimuth` L1 annotations; similar patterns observed with `SingleR`, `CellTypist`, and `scArches` by changing `query_cell_type_col` parameter
