...
This page explores annotation quality and disease mechanisms in spatially-resolved MERFISH data from DSS-induced colitis. We examine fibroblasts across healthy and inflamed tissue, combining cell type annotations, anomaly detection, gene expression analysis, and spatial interaction patterns to understand inflammatory remodeling.
...
- **Note:** Results below use tier2_merged annotations; similar patterns observed with alternative annotation methods by changing `query_cell_type_col` parameter

## Analysis Components

### 1. Spatial Cell Type and Fibroblast State Maps (Figure A)

**Script:** `R/merfish/Spatial Plots.R`
...
**Script:** `R/merfish/PCA ECM Plot.R` and `R/merfish/PCA Anomaly Plot.R`
...
**Script:** `R/merfish/Spatial Enrichment Plot.R`
...
**Script:** `R/merfish/Network Plot.R`
...
**Why these are not misclassified cells:**

- Spatial clustering indicates coordinated response to local signals
- ECM suppression is biological adaptation to chronic inflammation (reduced structural protein production)
- Proximity to damage suggests active participation in wound response
- Ligand-receptor network shows functional engagement with inflammatory milieu
