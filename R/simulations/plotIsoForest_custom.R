...
#' 2. Diagonal facets: Density, ridge, or boxplot visualizations showing the distribution of each PC, separated by anomaly status
...
#' @param tile_colors Colors for tiles. If NULL (default) the default tile colors from `scDiagnostics` are used.
#' @param tile_stops Stops for tiles. If NULL (default) the default tile stops from `scDiagnostics` are used.
...
plotIsoForest_custom <- function(x,
                                 cell_type = NULL,
                                 pc_subset = NULL,
    ...
    # Get reference data subset for the specified PCs
    reference_subset <- x[[cell_type]][["reference_mat_subset"]][, pc_subset, drop = FALSE]
    
    # Calculate combined ranges from both reference and query data (if available)
    combined_ranges <- list()
    for(i in seq_along(pc_subset)){
        pc_idx <- pc_subset[i]
        
        # Start with reference data range
        ref_range <- range(reference_subset[, i], na.rm = TRUE)
        
        # Add query data range if available
        if(!is.null(x[[cell_type]][["query_mat_subset"]])){
            query_range <- range(x[[cell_type]][["query_mat_subset"]][, pc_idx], na.rm = TRUE)
            combined_range <- range(c(ref_range, query_range), na.rm = TRUE)
        } else {
            combined_range <- ref_range
        }
        
        combined_ranges[[paste0("PC", pc_idx)]] <- combined_range
    }
    ...
    # Create anomaly labels based on data_type
    data_type_label <- ifelse(data_type == "query", "Query", "Reference")
    anomaly_labels <- ifelse(anomaly, 
                             paste0("Anomalous ", data_type_label, " Cell"),
                             paste0("Non-Anomalous ", data_type_label, " Cell"))
    pc_df$anomaly <- factor(anomaly_labels)
    
    # Colors for anomalous and non-anomalous data
    anomaly_colors <- c()
    anomaly_colors[paste0("Anomalous ", data_type_label, " Cell")] <- "red"
    anomaly_colors[paste0("Non-Anomalous ", data_type_label, " Cell")] <- "black"
    
    anomaly_fill_colors <- c()
    anomaly_fill_colors[paste0("Anomalous ", data_type_label, " Cell")] <- "#FFB6B6"
    anomaly_fill_colors[paste0("Non-Anomalous ", data_type_label, " Cell")] <- "#DDDDDD"
        ...
        for(j in seq_along(pc_subset)){
            if(i < j){
                ...
                if(data_type == "reference"){
                    train_data <-
                        x[[cell_type]][["reference_mat_subset"]][, c(pc_subset[i], pc_subset[j])]
                } else {
                    train_data <-
                        x[[cell_type]][["reference_mat_subset"]][, c(pc_subset[i], pc_subset[j])]
                }
    ...
    .anomalyScatterFunc <- function(data, mapping, ...){
        ...
        # Use combined ranges instead of just data ranges
        x_range <- combined_ranges[[paste0("PC", pc_x)]]
        y_range <- combined_ranges[[paste0("PC", pc_y)]]
        x_buffer <- 0.1 * diff(x_range)
        y_buffer <- 0.1 * diff(y_range)
        
        # Define the actual plot limits
        x_limits <- c(x_range[1] - x_buffer, x_range[2] + x_buffer)
        y_limits <- c(y_range[1] - y_buffer, y_range[2] + y_buffer)
        
        # Create grid for background that exactly matches the plot limits
        x_seq <- seq(x_limits[1], x_limits[2], length.out = 150)
        y_seq <- seq(y_limits[1], y_limits[2], length.out = 150)
        ...
        # Get anomaly scores for grid - need to use original PC column indices
        ...
        if(is.null(tile_colors)){
            ...
            if(length(tile_stops) != (length(tile_colors) + 1))
                stop("The length of \'\' does not match the requirement for \'tile_colors\'.")
            
            gradient_colors <- tile_colors
            stops <- tile_stops
        }
    ...
    .densityDiagFunc <- function(data, mapping, ...){
        ...
        # Get PC number from column name for range
        pc_num <- as.numeric(sub("PC([0-9]+).*", "\\1", x_name))
        x_range <- combined_ranges[[paste0("PC", pc_num)]]
    ...
    .contourFunc <- function(data, mapping, ...){
        ...
        # Get PC numbers from column names for ranges
        pc_x <- as.numeric(sub("PC([0-9]+).*", "\\1", x_name))
        pc_y <- as.numeric(sub("PC([0-9]+).*", "\\1", y_name))
        x_range <- combined_ranges[[paste0("PC", pc_x)]]
        y_range <- combined_ranges[[paste0("PC", pc_y)]]
        ...
        data_normal <- data[data$anomaly == paste0("Non-Anomalous ", data_type_label, " Cell"),]
        if(nrow(data_normal) >= 10) {
            p <- p + ggplot2::stat_density_2d(
                ...
                color = anomaly_colors[paste0("Non-Anomalous ", data_type_label, " Cell")],
                linewidth = 0.5,
                contour = TRUE,
                bins = 5
            )
        ...
        data_anomaly <- data[data$anomaly == paste0("Anomalous ", data_type_label, " Cell"),]
        if(nrow(data_anomaly) >= 10) {
            p <- p + ggplot2::stat_density_2d(
                ...
                color = anomaly_colors[paste0("Anomalous ", data_type_label, " Cell")],
                linewidth = 0.5,
                contour = TRUE,
                bins = 5
            )
    ...
    plot_obj <- suppressMessages(suppressMessages(
        GGally::ggpairs(
            pc_df,
