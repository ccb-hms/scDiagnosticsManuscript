...
#' @param cell_type_colors Vector of colors for the cell types.
#'
...
plotCellTypePCA_custom <- function(
        query_data,
        reference_data,
        query_cell_type_col,
        ref_cell_type_col,
    ...
    if(is.null(cell_types)){
        # Get reference cell types (no misannotated entries should exist in reference)
        ref_cell_types <- na.omit(unique(reference_data[[ref_cell_type_col]]))
        
        # Get query cell types and extract base cell types from misannotated ones
        query_cell_types <- na.omit(unique(query_data[[query_cell_type_col]]))
        
        # Extract base cell types from misannotated query entries
        query_base_types <- character()
        for(ct in query_cell_types) {
            if(grepl("\\n Misannotated as", ct)) {
                # Extract the target cell type (after "Misannotated as ")
                target_match <- regmatches(ct, regexpr("\\n Misannotated as (.+)$", ct))
                if(length(target_match) > 0) {
                    target_type <- gsub("\\n Misannotated as ", "", target_match)
                    query_base_types <- c(query_base_types, target_type)
                }
            } else {
                query_base_types <- c(query_base_types, ct)
            }
        }
        
        # Combine and get unique base cell types
        cell_types <- unique(c(ref_cell_types, query_base_types))
    }
    
    # Get the projected PCA data
    pca_output <- scDiagnostics::projectPCA(query_data = query_data,
                                            reference_data = reference_data,
                                            query_cell_type_col = query_cell_type_col,
                                            ref_cell_type_col = ref_cell_type_col,
                                            pc_subset = pc_subset,
    ...
    # Filter pca_output to include relevant cell types
    # For reference: only base cell types
    # For query: base cell types + misannotated entries that target base cell types
    keep_rows <- logical(nrow(pca_output))
    
    for(i in seq_len(nrow(pca_output))) {
        if(pca_output[["dataset"]][i] == "Reference") {
            # Keep if it's a base cell type
            keep_rows[i] <- pca_output[["cell_type"]][i] %in% cell_types
        } else {
            # Query data: keep base types and misannotated entries targeting base types
            if(pca_output[["cell_type"]][i] %in% cell_types) {
                keep_rows[i] <- TRUE
            } else if(grepl("\\n Misannotated as", pca_output[["cell_type"]][i])) {
                # Check if the target type is in our base cell types
                target_match <- regmatches(pca_output[["cell_type"]][i], 
                                           regexpr("\\n Misannotated as (.+)$", 
                                                   pca_output[["cell_type"]][i]))
                if(length(target_match) > 0) {
                    target_type <- gsub("\\n Misannotated as ", "", target_match)
                    keep_rows[i] <- target_type %in% cell_types
                }
            }
        }
    }
    
    pca_output <- pca_output[keep_rows, ]
    ...
    # Identify misannotated entries and create shape mapping
    is_misannotated <- grepl("\\n Misannotated as", cell_type_dataset)
    shape_values <- ifelse(is_misannotated, 4, 16)  # 4 = cross, 16 = circle
    
    # Create ordered combinations with misannotated entries placed after their targets
    base_cell_types <- sort(cell_types)  # These are the base cell types only
    order_combinations <- c()
    
    # Get all misannotated entries to organize them
    all_misannotated <- cell_type_dataset[is_misannotated]
    
    for(ct in base_cell_types) {
        # Add reference entry
        order_combinations <- c(order_combinations, paste("Reference", ct))
        # Add query entry  
        order_combinations <- c(order_combinations, paste("Query", ct))
        
        # Add misannotated entries that target this cell type
        misannotated_for_this_type <- all_misannotated[
            grepl(paste0("\\n Misannotated as ", ct, "$"), all_misannotated)]
        
        if(length(misannotated_for_this_type) > 0) {
            order_combinations <- c(order_combinations, unique(misannotated_for_this_type))
        }
    }
    ...
    color_mapping <- cell_type_colors
    names(color_mapping) <- order_combinations
    
    # Add the cell type dataset column and shape to the PC data frame
    pc_df[["cell_type_dataset"]] <- cell_type_dataset
    pc_df[["shape_type"]] <- shape_values
    
    # Filter data for upper/lower facets (exclude misannotated)
    pc_df_filtered <- pc_df[!is_misannotated, ]
    ...
    legend_plot <- ggplot2::ggplot(pc_df,
        ...
        ggplot2::scale_color_manual(values = color_mapping,
                                    name = "Cell Type") +
        ggplot2::scale_shape_manual(values = c("16" = 16, "4" = 4),
                                    guide = "none") +
        ggplot2::theme(
        ...
        ggplot2::guides(color = ggplot2::guide_legend(
            ncol = 1,
            override.aes = list(
                shape = ifelse(grepl("Misannotated", 
                                     levels(cell_type_dataset)), 4, 16))))
    
    # Scatterplot facet function
    # Scatterplot facet function
    .scatterFunc <- function(data, mapping, ...) {
        # For scatter plots, use full data (including misannotated) but with shapes
        p <- ggplot2::ggplot(data = data, mapping = mapping)
        
        # Add circles (shape 16) with normal stroke
        circle_data <- data[data$shape_type == 16, ]
        if(nrow(circle_data) > 0) {
            p <- p + ggplot2::geom_point(data = circle_data,
                                         alpha = 0.5, size = 1,
                                         ggplot2::aes(color = cell_type_dataset),
                                         shape = 16)
        }
        
        # Add crosses (shape 4) with thicker stroke
        cross_data <- data[data$shape_type == 4, ]
        if(nrow(cross_data) > 0) {
            p <- p + ggplot2::geom_point(data = cross_data,
                                         alpha = 0.5, size = 1, stroke = 1,
                                         ggplot2::aes(color = cell_type_dataset),
                                         shape = 4)
        }
        
        p + ggplot2::scale_color_manual(values = color_mapping) +
    ...
    .smoothContourFunc <- function(data, mapping, ...) {
        # Use filtered data for contour plots
        if(identical(deparse(substitute(data)), "pc_df")) {
            data <- pc_df_filtered
        }
    ...
    .robustEllipseFunc <- function(data, mapping, ...) {
        # Use filtered data for ellipse plots
        if(identical(deparse(substitute(data)), "pc_df")) {
            data <- pc_df_filtered
        }
    ...
    # Ridge diagonal facet (includes misannotated data)
    ...
    # Density diagonal facet (includes misannotated data)
    ...
    # Boxplot diagonal facet (includes misannotated data)
