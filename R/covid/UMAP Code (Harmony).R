# ------------------------------------------------
# COVID-19 PBMC - UMAP Visualization (Harmony)
# ------------------------------------------------
...
cat("COVID-19 UMAP VISUALIZATION (Harmony)\n")
cat(paste(rep("=", 45), collapse = ""), "\n")
...
# Find common genes for compatibility
...
# For normal data, we use its own ground truth for both original and hybrid labels
...
# For COVID data, we include its Azimuth prediction to create the hybrid label
...
# Combine the data frames using bind_rows for safety
...
cat(sprintf("Using %d/%d diagnostic HVGs for PCA.\n", length(available_hvgs), length(diagnostic_hvgs)))
