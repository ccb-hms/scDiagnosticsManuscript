# -----------------------------------------------
# COVID-19 PBMC - Supplementary Figure 5 Code
# -----------------------------------------------
...
# Figure S5A: Azimuth Confidence Scores
# ______________________________________
...
fig_s5a <- ggplot(plot_data_azimuth, aes(x = ConfidenceScore, fill = Anomaly)) +
    ...
    theme(
        ...
        strip.text = element_text(size = 18, face = "bold", color = "#1F2937", family = "sans"),
...
# ______________________________________
# Figure S5B: SingleR Delta Distribution
# ______________________________________
...
fig_s5b <- ggplot(delta_data, aes(x = Delta, fill = Anomaly)) +
    geom_density(alpha = 0.8, linewidth = 1, color = "white") +
    facet_wrap(~Anomaly, ncol = 1, labeller = labeller(Anomaly = c(
        "Anomalous" = "Anomalous CD14+ Monocytes",
        "Non-anomalous" = "Non-Anomalous CD14+ Monocytes"
...
# Figure S5C: CellTypist Confidence Scores
# _________________________________________
...
fig_s5c <- ggplot(plot_data_celltypist, aes(x = ConfidenceScore, fill = Anomaly)) +
    geom_density(alpha = 0.8, linewidth = 1, color = "white") +
    facet_wrap(~Anomaly, ncol = 1, labeller = labeller(Anomaly = c(
        "Anomalous" = "Anomalous CD14+ Monocytes",
        "Non-anomalous" = "Non-Anomalous CD14+ Monocytes"
...
# Figure S5D: scArches Confidence Scores
# _______________________________________
...
fig_s5d <- ggplot(plot_data_scarches, aes(x = ConfidenceScore, fill = Anomaly)) +
    geom_density(alpha = 0.8, linewidth = 1, color = "white") +
    facet_wrap(~Anomaly, ncol = 1, labeller = labeller(Anomaly = c(
        "Anomalous" = "Anomalous CD14+ Monocytes",
        "Non-anomalous" = "Non-Anomalous CD14+ Monocytes"
...
fig_s5_combined <- plot_grid(fig_s5a, fig_s5b,
                             fig_s5c, fig_s5d,
                             nrow = 2, ncol = 2, labels = "AUTO",
                             label_size = 26, label_fontface = "bold")

ggsave("figures/supp/covid/Fig_S5_confidence_scores.png", fig_s5_combined, width = 20, height = 18, dpi = 600)
...
# Table S5: Wilcoxon Tests - Anomaly vs Confidence
# ___________________________________________________
...
# Remove NULL entries
table_s5_data <- do.call(rbind, Filter(Negate(is.null), table_s5_list))
rownames(table_s5_data) <- NULL
print(table_s5_data)
