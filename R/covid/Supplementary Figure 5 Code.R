...
create_geneshifts_barplot <- function(gene_shifts_obj, cell_type, genes_order, method_name) {
    ...
    p <- ggplot(plot_data, aes(x = log2fc, y = factor(gene, levels = rev(genes_avail)), fill = group)) +
        ...
        theme(
            ...
            axis.title = element_text(size = 10, face = "bold", color = "#374151", family = "sans"),
...
fig_s5_3a <- create_quantile_gene_plot(cd14_celltypist, normal_cd14_singler, yoshida_ifn_signature, 
                                        "CellTypist", "celltypist_conf_score",
                                        score_direction = "higher_is_better",
                                        gene_order = rev(gene_order_celltypist))

fig_s5_3b <- create_geneshifts_barplot(gene_shifts_celltypist, "CD14_mono", gene_order_celltypist, "CellTypist")
...
fig_s5_4a <- create_quantile_gene_plot(cd14_scarches, normal_cd14_singler, yoshida_ifn_signature, 
                                        "scArches", "scvi_confidence",
                                        score_direction = "lower_is_better",
                                        gene_order = rev(gene_order_scarches))

fig_s5_4b <- create_geneshifts_barplot(gene_shifts_scarches, "CD14_mono", gene_order_scarches, "scArches")
...
table_s5_list <- list(
    ...
    create_correlation_row(gene_order_scarches, cd14_scarches, normal_cd14_singler,
                          gene_shifts_scarches, "scArches", "scvi_confidence",
                          score_direction = "lower_is_better")
)

table_s5 <- do.call(rbind, Filter(Negate(is.null), table_s5_list))
rownames(table_s5) <- NULL
print(table_s5)
