...
reference <- AzimuthReference(
    ...
    reference.version = paste0("custom_reference_", ncol(normal_seurat), "cells_v1.0"),
    verbose = FALSE
)
...
reference_info <- list(
    ...
    n_genes = nrow(normal_seurat),
    cell_types = levels(normal_seurat$cell_type),
    creation_date = Sys.time(),
    version = paste0("custom_reference_", ncol(normal_seurat), "cells_v1.0"),
...
# Cleanup
rm(normal_data, normal_seurat, reference, counts_data, logcounts_data, metadata, input_sce)
gc()
